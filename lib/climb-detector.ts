...
export function detectClimbs(
  ...
  for (let i = 1; i < smoothedPoints.length; i++) {
    ...
    const gradient =
      distance > 0 ? (elevationChange / (distance * 1000)) * 100 : 0;
    ...
    // End of climb (gradient drops below threshold or we reach the end)
      ...
      isClimbing &&
      (gradient < gradientThreshold || i === smoothedPoints.length - 1)
    ) {
      if (climbStart !== null && climbStartElevation !== null) {
        ...
        if (
          climbLength >= minClimbLength &&
          elevationGain >= minElevationGain
        ) {
  ...
  for (let i = 0; i < climbs.length; i++) {
    ...
    if (merged.length > 0) {
      ...
      const distanceBetween =
        currentClimb.startDistance - lastClimb.endDistance;
