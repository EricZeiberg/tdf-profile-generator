...
export const ElevationProfile = forwardRef<HTMLCanvasElement, ElevationProfileProps>(
  ({ gpxData, labels, onProfileClick, isAddingLabel, startName, finishName }, ref) => {
    ...
    useEffect(() => {
      ...
      ctx.fillRect(padding.left, padding.top + chartHeight + 20, chartWidth, 30)
      ...
      labels.forEach((label) => {
        ...
        ctx.fillText(`${label.distance.toFixed(1)}`, x, padding.top + chartHeight + 40)
      })

      // Always show start (0) and end markers
      ctx.fillText("0", xScale(0), padding.top + chartHeight + 40)
      ctx.fillText(`${maxDistance.toFixed(1)}`, xScale(maxDistance), padding.top + chartHeight + 40)
      ...
      ctx.fillText(startName.toUpperCase(), padding.left, 20)
      ctx.font = "12px Arial, sans-serif"
      ctx.fillText(`${startElevation.toFixed(0)} m`, padding.left, 35)

      // End label
      ctx.textAlign = "right"
      ctx.font = "bold 14px Arial, sans-serif"
      const endElevation = gpxData.elevationPoints[gpxData.elevationPoints.length - 1]?.elevation || minElevation
      ctx.fillText(finishName.toUpperCase(), containerWidth - padding.right, 20)
      ctx.font = "12px Arial, sans-serif"
      ctx.fillText(`${endElevation.toFixed(0)} m`, containerWidth - padding.right, 35)
      ctx.fillText(`${maxDistance.toFixed(1)} km`, containerWidth - padding.right, 50)
      ...
      ctx.textAlign = "right"
      ctx.fillText(`${maxDistance.toFixed(1)} km`, containerWidth - 20, containerHeight - 10)
    ...
    const handleCanvasClick = (event: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (x < padding.left || x > containerWidth - padding.right || y < padding.top || y > padding.top + chartHeight) {
        return
      }
    ...
    return (
      <div ref={containerRef} className="w-full">
        <canvas
          ...
          className={`w-full border border-gray-200 rounded ${isAddingLabel ? "cursor-crosshair" : "cursor-default"}`}
          style={{ height: "400px" }}
        />
        {isAddingLabel && (
