...
export default function GPXElevationProfiler() {
    ...
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        const autoLabels: LabelPoint[] = climbs
          .filter(
            (climb) =>
              climb.category && ["HC", "1", "2", "3"].includes(climb.category)
          )
          .map((climb) => ({
            ...
            averageGradient: climb.averageGradient,
          }));
  ...
  const handleProfileClick = useCallback(
    (distance: number, elevation: number) => {
      ...
      setLabels((prev) =>
        [...prev, newLabel].sort((a, b) => a.distance - b.distance)
      );
      setNewLabelName("");
  ...
  const handleLabelUpdate = useCallback(
    (labelId: string, updates: Partial<LabelPoint>) => {
      setLabels((prev) =>
        prev.map((label) =>
  ...
  const downloadImage = useCallback(() => {
    ...
    // Hide outlines for clean image
    setHideOutlines(true);

    // Wait for re-render, then download
    setTimeout(() => {
      if (!canvasRef.current) return;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-gray-600">
            Upload a GPX file to generate a Tour de France-style elevation
            profile
          </p>
        </div>
          ...
          <div className="lg:col-span-1 space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Select a GPX file to generate the elevation profile
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            {detectedClimbs.length > 0 && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Automatically detected climbs from your GPX data
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {detectedClimbs.map((climb, index) => {
                      const isAlreadyAdded = labels.some(
                        (label) =>
                          Math.abs(label.distance - climb.peakDistance) < 0.1
                      );
                      return (
                        <div
                          key={index}
                          ...
                          onMouseEnter={() =>
                            !isAlreadyAdded && setHoveredClimb(climb)
                          }
            ...
            {gpxData && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Rename the start and finish points
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
            ...
            {gpxData && (
              <Button
                ...
                className="w-full whitespace-normal text-wrap"
              >
            ...
            {gpxData && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Manage custom labels and automatically generated climb
                    markers
                  </CardDescription>
                </CardHeader>
                <CardContent>
                    ...
                    <div className="space-y-4">
                      ...
                      {isAddingLabel && (
                        <p className="text-sm text-blue-600 text-center">
                          Click on the elevation profile above to add the label
                          at that location
                        </p>
                      )}
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      {labels.length > 0 ? (
                        ...
                        <p className="text-sm text-gray-500 text-center py-4">
                          No labels added yet. Add custom labels or use detected
                          climbs above.
                        </p>
                      )}
                    </div>
